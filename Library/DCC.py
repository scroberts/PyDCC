...
def user_search(s, **kwargs):

    url = DCC_URL + "/dsweb/SEARCH"
    headers = {"DocuShare-Version":"5.0", "Content-Type":"text/xml", "Accept":"text/xml"}
    ...
    if 'email' in kwargs:
        xml2 = '''<where><contains><prop><email/></prop><literal><![CDATA[''' + kwargs.get('email') + ''']]></literal></contains></where>'''

    elif 'phone' in kwargs:
        xml2 = '''<where><eq><prop><phone/></prop><literal><![CDATA[''' + kwargs.get('phone') + ''']]></literal></eq></where>'''

    elif 'username' in kwargs:
        xml2 = '''<where><eq><prop><username/></prop><literal><![CDATA[''' + kwargs.get('username') + ''']]></literal></eq></where>'''
    

    elif 'name' in kwargs:
        first_name = kwargs.get('name')[0]
        last_name = kwargs.get('name')[1]
        print(first_name, last_name)
        
        xml2 = '''<where><and>''' \
                '''<eq><prop><first_name/></prop><literal>''' + first_name + '''</literal></eq>''' \
                '''<eq><prop><last_name/></prop><literal>''' + last_name + '''</literal></eq>''' \
                '''</and></where>'''
    else:
        print('Unknown input argument')
        exit(1)

    xml3 = '''</simplesearch></searchrequest>'''
    xml = xml1 + xml2 + xml3
    r = s.post(url,data=xml,headers=headers)
#     print("Search status code:", r.status_code)
#     print("Search r.text", r.text)
#     print("header r.json", r.json)
    dom = BeautifulSoup(r.text, "html.parser")
    
    ulist = []
    for res in dom.find_all("response"):
        ud = {}
        path = res.href.text.split('/')[-1]
        ud['handle'] = path
        ud['username'] = res.username.text        
        ud['first_name'] = res.first_name.text
    ...
    FileSys.file_write_props(r, 'search_test')
...
#     print(ulist)
    ...
    print('DCC_URL:', DCC_URL)    
    prop_get(s, handle = 'User-513', InfoSet = 'User', Print = True)
    
    ulist = user_search(s, phone = '250/363-8314')
    print(ulist)

    ulist = user_search(s, name = ['Scott', 'Roberts'])
    print(ulist)
    
    ulist = user_search(s, username = 'hthompson')
    print(ulist)
    
    ulist = user_search(s, name = ['John', 'Rogers'])
    print(ulist)
